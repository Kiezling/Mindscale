...
- [ ] Remaining Insights views after `SPEC-insights-foundation.md` — build on the verified shared episode engine with a step-only entry chart and sleep/event overlays, then consider an onset-gap histogram, onset-time counts with sample refusal/caveat, and descriptive sleep counts under a separately approved bounded spec.
