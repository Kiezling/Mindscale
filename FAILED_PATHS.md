...
- Decision/workaround: do not pass a long body inline. Write it to a file with the Write tool, then reference it: `gh pr create ... --body-file <path>`. This is how PR #10 was actually created after the inline attempt was blocked. The same applies to any long prose argument — commit messages that trip it can use `git commit -F <file>`.
